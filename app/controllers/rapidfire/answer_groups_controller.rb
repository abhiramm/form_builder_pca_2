module Rapidfire
  class AnswerGroupsController < Rapidfire::ApplicationController
    ...
    def new
      ...
      @parent = @device.parent if @device
    ...
    def create
     ...
     # @answer_group_builder.is_read = false
      if @answer_group_builder.save
     	@current_answer_group = AnswerGroup.find_by_id(@answer_group_builder.answers.first.answer_group_id)
	Rails.logger.info "current_answer_group #{@current_answer_group}"
	@current_answer_group.parent_id = params[:parent_id]
	@current_answer_group.save
